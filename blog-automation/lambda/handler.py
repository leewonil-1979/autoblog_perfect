...
# - 매일 EventBridge가 호출
# - 1) 주제 생성 2) 초안 작성 3) 최적화 4) SEO 렌더 5) 워드프레스 발행 or 티스토리 패키지(S3) 6) 로그/DB 기록
# - DB는 외부(PostgreSQL on Docker/EC2)로 접속
...
# ===== LLM 선택 =====
def model(stage: str):
    # 비용 최적화: 요약/벤치=Claude, 집필/검수=GPT-4.x mini (여기서는 GPT-5 자리에 gpt-4.1-mini 사용 예)
    if stage in {"topic","bench"} and ANTHROPIC_API_KEY:
        return ChatAnthropic(model="claude-3-5-haiku-latest", anthropic_api_key=ANTHROPIC_API_KEY, temperature=0.2, max_tokens=800)
    return ChatOpenAI(model="gpt-4.1-mini", api_key=OPENAI_API_KEY, temperature=0.3, max_tokens=1800)

# ===== 유틸 =====
def db_conn():
    return psycopg2.connect(DATABASE_URL)

def log_exec(blog_id: int, step: str, status: str, message: str = "", cost: float = 0.0):
    try:
...
def fetch_active_blogs() -> List[Dict[str,Any]]:
    with db_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT id, blog_name, blog_url, platform, wp_user, wp_app_password, COALESCE(category,'') FROM blogs WHERE active = TRUE")
            rows = cur.fetchall()
    keys = ["id","blog_name","blog_url","platform","wp_user","wp_app_password","category"]
    return [dict(zip(keys,r)) for r in rows]

def wp_publish(blog, title: str, html: str) -> Dict[str,Any]:
    url = blog["blog_url"].rstrip("/") + "/wp-json/wp/v2/posts"
    # Application Password → 기본 인증
...
def s3_upload_text(key: str, text: str) -> str:
    ...
    s3.put_object(Bucket=S3_BUCKET_TISTORY, Key=key, Body=text.encode("utf-8"), ContentType="text/html; charset=utf-8")
    return f"s3://{S3_BUCKET_TISTORY}/{key}"

def save_article(blog_id: int, title: str, content: str, html: str, status: str, wp_id: int | None, tistory_s3: str | None):
    with db_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
...
def handler(event, context):
    ...
    results = []
    for blog in blogs:
        b_id = blog["id"]; b_name = blog["blog_name"]
        try:
            # 1) 주제 생성
            llm = model("topic")
            topic = llm.invoke(f"블로그 '{b_name}' 주제로 한국어 트렌디 글 주제 1개만 제시. 12~20자.").content.strip()
            log_exec(b_id, "topic_generation", "success", topic)

            # 2) 초안 작성
            writer = model("draft")
            draft = writer.invoke(f"""
주제: {topic}
...
""").content
            ...
            title = topic  # 간단화; 필요시 추가 추출
            # 3) SEO 렌더(메타/스타일/CTA/IMG 자리 포함)
            rendered = render_html(topic=title, intent="정보", outline=["개요","핵심 단계","사례"], images=4)
            html = rendered["html"].replace("<h1>"+title+"</h1>", f"<h1>{title}</h1>\n{draft}")
            ...
            if blog["platform"] == "wordpress":
                ...
                results.append({"blog": b_name, "platform":"wordpress", "post_id": wp.get("id")})
            else:
                key = f"{b_id}/{int(time.time())}_{title}.html"
                s3_uri = s3_upload_text(key, html)
                save_article(b_id, title, draft, html, "draft", None, s3_uri)
                log_exec(b_id, "tistory_package", "success", s3_uri)
                results.append({"blog": b_name, "platform":"tistory", "package": s3_uri})
